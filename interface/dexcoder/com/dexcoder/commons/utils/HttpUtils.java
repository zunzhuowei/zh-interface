...
import com.dexcoder.commons.mvc.HttpContext;
...
public class HttpUtils {
    ...
    public static String getRemoteAddr(HttpServletRequest req) {
        ...
        if(StrUtils.isNotBlank(ip)){
            String[] ips = StrUtils.split(ip,',');
            if(ips!=null){
                for(String tmpip : ips){
                    if(StrUtils.isBlank(tmpip))
                        continue;
                    tmpip = tmpip.trim();
    ...
    public static boolean isRobot(HttpServletRequest req){
        ...
        if(StrUtils.isBlank(ua)) return false;
        return (ua != null
     ...
     * @param name
     */
    public static Cookie getCookie(HttpContext hc, String name) {
        Cookie[] cookies = hc.request.getCookies();
        if(cookies == null) return null;
        for (Cookie ck : cookies) {
            if (StrUtils.equalsIgnoreCase(name,ck.getName())) 
                return ck;    
        }
        return null;
    }
     ...
     * @param name
     */
    public static String getCookieValue(HttpContext hc, String name) {
        Cookie[] cookies = hc.request.getCookies();
        if(cookies == null) return null;
        for (Cookie ck : cookies) {
            if (StrUtils.equalsIgnoreCase(name,ck.getName())) 
     ...
     * @param maxAge
     */
    public static void setCookie(HttpContext hc, String name,
            String value, int maxAge) {
        setCookie(hc.request,hc.response,name,value,maxAge,true);
    }
    ...
    public static void editCookie(HttpContext hc,String name,String value,int maxAge){
    	Cookie cookie = new Cookie(name, value);
    ...
    public static String getDomainOfServerName(String host){
        if(isIPAddr(host))
            return null;
        String[] names = StrUtils.split(host, '.');
    ...
    public static boolean isIPAddr(String addr){
        if(StrUtils.isEmpty(addr))
            return false;
        String[] ips = StrUtils.split(addr, '.');
        if(ips.length != 4)
            return false;
        try{
    ...
    public static String getParam(HttpServletRequest req, String param, String defaultValue){
        ...
        return (StrUtils.isEmpty(value))?defaultValue:value;
    }
